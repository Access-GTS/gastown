...
func (m *Manager) AllocateName() (string, error) {
	...
	if m.tmux != nil {
		sessionName := session.PolecatSessionName(session.PrefixForRig(m.rig.Name), name)
...
func (m *Manager) DetectStalePolecats(threshold int) ([]*StalenessInfo, error) {
	...
	for _, p := range polecats {
		...
		sessionName := session.PolecatSessionName(session.PrefixForRig(m.rig.Name), p.Name)
